...
fn load_test_data() -> Vec<String> {
    ...
    let aws_keys = vec![
        ...
        "AKIAI44QH8DHCAMPLE",
        "AKIAIOSFODNN9EXAMPLE",
        "AKIAI44QH8DHDAAMPLE",
        "AKIAIOSFODNNAEXAMPLE",
        "AKIAI44QH8DHDAAMMPLE",
        "AKIAIOSFODNNBEXAMPLE",
        "AKIAI44QH8DHDAAAMPLE",
    ];
    ...
    for i in 0..100 {
        ...
        // SSH Key
        texts.push(ssh_keys[idx].to_string());
        texts.push(format!("Public key: {}", ssh_keys[(idx + 1) % 10]));

        // Комбинированные тексты
        texts.push(format!(
            "User {} with phone {} and IP {}",
            emails[idx], phones[idx], ips[idx]
        ));
        texts.push(format!(
            "Contact: {}, Phone: {}",
            emails[(idx + 3) % 10],
