...
## Выборочная анонимизация параметров

По умолчанию PII Anonymizer анонимизирует **все** строковые значения
в аргументах проксируемых инструментов. Это можно настроить в
...
proxy:
  upstream_servers:
    searxng:
      ...
      anonymize_fields:
        searxng_web_search: ["query"]   # анонимизировать только поисковый запрос
        web_url_read: []                 # [] = не анонимизировать вообще
```

Если `anonymize_fields` не указан — анонимизируются все строки.
Если указано пустой список `[]` — анонимизация отключена для этого инструмента.
