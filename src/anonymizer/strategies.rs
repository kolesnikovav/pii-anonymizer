...
impl AnonymizationStrategy {
    ...
    fn partial_mask(&self, value: &str) -> String {
        ...
        if let Some(at_pos) = value.find('@') {
            ...
            let local_prefix = if local_part.len() >= 2 {
                &local_part[..2]
            } else {
                local_part
            };
    ...
    fn partial_hash(&self, value: &str) -> String {
        ...
        if len <= 4 {
            return format!(
                "_{}",
                self.simple_hash(value).chars().take(6).collect::<String>()
            );
        }
    ...
    fn get_mask_boundaries(&self, value: &str, len: usize) -> (usize, usize) {
        ...
        if value.contains('@') {
            if let Some(at_pos) = value.find('@') {
                ...
                let suffix_len = if domain_part.len() >= 2 {
                    2
                } else {
                    domain_part.len()
                };
...
mod tests {
    ...
    fn test_replace_strategy() {
        ...
        assert_eq!(
            strategy.apply("192.168.1.1", "ip_address", 3),
            "[IP_ADDRESS_3]"
        );
    }
    ...
    fn test_from_str() {
        assert_eq!(
            AnonymizationStrategy::parse_strategy("replace"),
            AnonymizationStrategy::Replace
        );
        assert_eq!(
            AnonymizationStrategy::parse_strategy("mask"),
            AnonymizationStrategy::Mask
        );
        assert_eq!(
            AnonymizationStrategy::parse_strategy("hash"),
            AnonymizationStrategy::Hash
        );
        assert_eq!(
            AnonymizationStrategy::parse_strategy("unknown"),
            AnonymizationStrategy::Replace
        );
    }
