...
pub fn is_known_domain(domain: &str, custom_domains: &[String]) -> bool {
    ...
    if BUILTIN_KNOWN_DOMAINS
        .iter()
        .any(|known| domain_lower == *known || domain_lower.ends_with(&format!(".{}", known)))
    {
        return true;
    }
...
mod tests {
    ...
    fn test_jwt_token_pattern() {
        let patterns = get_all_patterns();
        let jwt_pattern = patterns
            .iter()
            .find(|p| p.name == "access_token_jwt")
            .unwrap();
    ...
    fn test_ssh_key_rsa_pattern() {
        ...
        assert!(ssh_pattern
            .pattern
            .is_match("ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQDVvvHkGphJbBX8"));
    }
